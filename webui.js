...
function setSettings()
{
	...
	query += "&devicename=" + document.getElementById("devicename").value;
...
function getFolders()
{
	utWebUI.getSyncFolders(function(c)
	{
		...
		$("#filetable tbody tr").each(function()
		{
			...
			{
				if (row.children('td').eq(nameIndex).text() == folder.name)
				{
		...
		$.each(folders, function(index, folder)
		{
			...
			var secretButton = $("<a>Get secret</a>").addClass("btn");
			var secretFunction = (function(s)
			{
				return function()
				{
					$("#getsecret").val(s);
